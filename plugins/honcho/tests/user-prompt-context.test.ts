/**
 * Unit tests for the context-selection helpers in src/hooks/user-prompt.ts.
 *
 * These cover the #63 injection-quality fix: prompt-matched conclusions are
 * injected first, remaining slots are filled with the NEWEST representation
 * lines (not the stalest head of the oldest-first representation), and the
 * fork's cross-turn `seen` dedup still holds. Ported/adapted from upstream
 * plastic-labs/claude-honcho#63, extended for this fork's `seen`/`newConclusions`
 * layer.
 */
import { describe, expect, test } from "bun:test";
import { extractTopics, formatCachedContext, stripConclusionLine } from "../src/hooks/user-prompt.ts";

/** Pull the injected conclusion list back out of the formatted parts. */
...
describe("formatCachedContext", () => {
  ...
  test("fills remaining slots with newest representation lines, not oldest", () => {
    // Regression for the core bug: the representation is ordered oldest-first
    // and the hook used to inject its head.
  ...
  // Fork-specific: the cross-turn `seen` layer (#39) survives the #63 rework.
  test("excludes conclusions already injected this session (seen), across matches and representation", () => {
    const ctx = { representation: rep, searchMatched: ["middle fact"] };
    // "middle fact" was injected on an earlier turn — suppress it everywhere.
    const { parts, newConclusions } = formatCachedContext(ctx, "peer", ["middle fact"]);
    expect(conclusions(parts)).toEqual(["newest fact", "oldest fact"]);
    // newConclusions is what gets recorded for the next turn's `seen`.
    expect(newConclusions).toEqual(["newest fact", "oldest fact"]);
  });

  test("suppresses all conclusions when every candidate is already seen", () => {
    const ctx = { representation: rep };
    const seen = ["newest fact", "middle fact", "oldest fact"];
    const { parts, conclusionCount, newConclusions } = formatCachedContext(ctx, "peer", seen);
    expect(parts.filter((p) => p.startsWith("Relevant conclusions"))).toHaveLength(0);
    expect(conclusionCount).toBe(0);
    expect(newConclusions).toEqual([]);
  });
});
